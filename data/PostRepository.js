...
var PostRepository = function ()
{
    ...
    function buildNextPrev( position, row, callback )
    {
        if ( row )
        {
            post[position] = {
    ...
    function getNextPrev( db, url, callback )
    {
                ...
                LIMIT 1", { $url: url }, function ( err, next )
        {
            ...
            buildNextPrev( "next", next, function ( post )
            {
                db.get( "SELECT Url, Title, LinkText \
                     FROM POSTS \
                     WHERE \
                     ...
                     LIMIT 1", { $url: url }, function ( err, prev )
                {
                    ...
                    buildNextPrev( "prev", prev, function ( post )
                    {
                        callback( post );
                    } );
                } );
        ...
        getLatest: function ( callback )
        {
                       ...
                       ORDER BY P.CreatedOn DESC", function ( err, rows )
            {
                if ( rows && rows.length > 0 )
                {
                    ...
                    getNextPrev( repo, rows[0].Url, function ( p )
                    {
                        callback( null, p );
                    } );
                }
